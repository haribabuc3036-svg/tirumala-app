...
/**
 * One TTD event entry scraped from news.tirumala.org/category/events/
 */
...
const RSS_EVENTS  = 'https://news.tirumala.org/category/events/feed/';

/**
 * Scrape upcoming/recent TTD events from news.tirumala.org/category/events/
 *
 ...
 */
export async function scrapeEvents(): Promise<EventsScrapeResult> {
  // ── Strategy 1: WP REST API ────────────────────────────────────────────────
  try {
    // Step 1a: resolve "events" category ID
    const { data: cats } = await http.get<WpCategory[]>(
      `${WP_API}/categories?slug=events&_fields=id,slug`,
    );

    const categoryId = Array.isArray(cats) && cats.length > 0 ? cats[0].id : null;

    if (categoryId != null) {
      // Step 1b: fetch posts in the events category
      ...
      if (Array.isArray(posts) && posts.length > 0) {
        const entries: EventEntry[] = posts
          .map((post) => ({
  ...
  try {
    const { data: xml } = await http.get<string>(RSS_EVENTS);
    ...
    return { success: false, data: [], error: 'Events RSS feed returned no items.' };
  } catch (err: unknown) {
