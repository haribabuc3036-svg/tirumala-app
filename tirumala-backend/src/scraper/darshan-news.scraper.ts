...
export interface DarshanNewsEntry {
  title: string;
  link:  string;
  date:  string;
}

export interface DarshanNewsScrapeResult {
  success: boolean;
  data:    DarshanNewsEntry[];
  error:   string | null;
}
...
const RSS_URL = 'https://news.tirumala.org/category/darshan/feed/';

export async function scrapeDarshanNews(): Promise<DarshanNewsScrapeResult> {
  try {
    const { data: cats } = await http.get<WpCategory[]>(
      `${WP_API}/categories?slug=darshan&_fields=id,slug`,
    );
    ...
    if (categoryId != null) {
      ...
      if (Array.isArray(posts) && posts.length > 0) {
        const entries: DarshanNewsEntry[] = posts
  ...
  try {
    ...
    const entries: DarshanNewsEntry[] = [];
    $('item').each((_, el) => {
    ...
    if (entries.length > 0) return { success: true, data: entries, error: null };
    return { success: false, data: [], error: 'Darshan RSS feed returned no items.' };
  } catch (err: unknown) {
