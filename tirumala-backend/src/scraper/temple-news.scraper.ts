...
export interface TempleNewsEntry {
  title: string;
  link:  string;
  date:  string;
}

export interface TempleNewsScrapeResult {
  success: boolean;
  data:    TempleNewsEntry[];
  error:   string | null;
}
...
const RSS_URL  = 'https://news.tirumala.org/category/temple/feed/';

export async function scrapeTempleNews(): Promise<TempleNewsScrapeResult> {
  try {
    const { data: cats } = await http.get<WpCategory[]>(
      `${WP_API}/categories?slug=temple&_fields=id,slug`,
    );
    ...
    if (categoryId != null) {
      ...
      if (Array.isArray(posts) && posts.length > 0) {
        const entries: TempleNewsEntry[] = posts
  ...
  try {
    ...
    if (entries.length > 0) return { success: true, data: entries, error: null };
    return { success: false, data: [], error: 'Temple RSS feed returned no items.' };
  } catch (err: unknown) {
