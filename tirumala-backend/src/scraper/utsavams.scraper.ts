...
/**
 * One TTD Utsavam entry scraped from news.tirumala.org/category/utsavams/
 */
export interface UtsavamEntry {
  title: string;           // event headline
  link:  string;           // URL to full article
  date:  string;           // ISO date string e.g. "2026-03-11T14:22:47"
}

export interface UtsavamsScrapeResult {
  success: boolean;
  data:    UtsavamEntry[];
  error:   string | null;
}
...
const RSS_UTSAVAMS = 'https://news.tirumala.org/category/utsavams/feed/';

/**
 * Scrape Utsavam news/updates from news.tirumala.org/category/utsavams/
 *
 ...
 */
export async function scrapeUtsavams(): Promise<UtsavamsScrapeResult> {
  // ── Strategy 1: WP REST API ────────────────────────────────────────────────
  try {
    const { data: cats } = await http.get<WpCategory[]>(
      `${WP_API}/categories?slug=utsavams&_fields=id,slug`,
    );
    ...
    if (categoryId != null) {
      ...
      if (Array.isArray(posts) && posts.length > 0) {
        const entries: UtsavamEntry[] = posts
          .map((post) => ({
  ...
  try {
    const { data: xml } = await http.get<string>(RSS_UTSAVAMS);
    const $ = cheerio.load(xml, { xmlMode: true });

    const entries: UtsavamEntry[] = [];
    $('item').each((_, el) => {
    ...
    return { success: false, data: [], error: 'Utsavams RSS feed returned no items.' };
  } catch (err: unknown) {
