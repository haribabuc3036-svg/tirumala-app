    ...
    <View style={[styles.card, { borderColor: accent + '25', backgroundColor: accent + '08' }]}>
      ...
      <View style={{ flex: 1, padding: 14, gap: 10 }}>
        ...
        <Animated.View style={{ opacity, gap: 6 }}>
          ...
          <View style={{ width: '80%', height: 13, backgroundColor: accent + '25', borderRadius: 6 }} />
          <View style={{ width: '60%', height: 13, backgroundColor: accent + '25', borderRadius: 6 }} />
        </Animated.View>
        <Animated.View style={[{ opacity }, { flexDirection: 'row', justifyContent: 'space-between' }]}>
...
function NewsCard({
  ...
  item: { text: string; link?: string; date?: string };
  idx: number;
  accent: string;
}) {
        ...
        <View style={styles.cardInner}>
          ...
          <View style={styles.cardTopRow}>
            ...
            <View style={[styles.iconChip, { backgroundColor: accent + '15' }]}>
              <MaterialCommunityIcons name="newspaper-variant-outline" size={12} color={accent} />
            </View>
          </View>

          <ThemedText style={styles.titleText}>{item.text}</ThemedText>
...
export default function LatestNewsScreen() {
  ...
  const { latestUpdates, loading } = useLiveUpdates();
  const items = latestUpdates.slice(0, MAX_ITEMS);
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={loading} tintColor={accent} colors={[accent]} />
        }>
        ...
        <LinearGradient
          ...
          <View style={styles.heroTopRow}>
            <View style={[styles.heroIconWrap, {
              ...
              <MaterialCommunityIcons name="newspaper-variant-outline" size={26} color={isDark ? '#fff' : '#000'} />
            </View>
            <View style={{ flex: 1 }}>
              <ThemedText style={[styles.heroTitle, { color: isDark ? '#fff' : '#000' }]}>Latest News</ThemedText>
              <ThemedText style={[styles.heroSubtitle, { color: isDark ? 'rgba(255,255,255,0.78)' : 'rgba(0,0,0,0.6)' }]}>Official TTD news &amp; updates</ThemedText>
            </View>
          </View>
          ...
          <View style={styles.heroCountRow}>
            <View style={[styles.countBadge, {
              ...
              <ThemedText style={[styles.countText, { color: isDark ? '#fff' : '#000' }]}>
                {loading ? 'Loadingâ€¦' : `${items.length} of ${latestUpdates.length} articles`}
              </ThemedText>
            </View>
        ...
        <View style={styles.listWrap}>
          ...
          {!loading && items.length === 0 ? (
            <View style={styles.emptyWrap}>
              ...
              <ThemedText style={styles.emptyTitle}>No news yet</ThemedText>
              <ThemedText style={styles.emptySubtitle}>
                Check back soon â€” TTD publishes updates regularly.
              </ThemedText>
            </View>
          ) : null}
          ...
          {!loading
            ? items.map((item, idx) => (
                <NewsCard key={`${item.date ?? ''}-${idx}`} item={item} idx={idx} accent={accent} />
              ))
            : null}
...
const styles = StyleSheet.create({
  ...
  countText: { fontSize: 11.5, fontWeight: '700', color: '#fff' },
  sourceBadgeHero: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  sourceHeroText: { fontSize: 11, color: 'rgba(255,255,255,0.7)', fontWeight: '500' },
